...
local function makeflag(node, flag)
    if flag then
...
local function makecontent(ret, name, node, typestr, content)
    ...
    if type(val) == "table" then
        for i, _content in ipairs(content) do
            if i ~= 1 then
                ...
                insert(ret, format("%s{%s} %s\n", name, makeflag(node, _flag),
                                   _val))
            end
        end
    else
        insert(ret, format("%s{%s} %s\n", name, makeflag(node, flag), val))
    end
end
...
return function(datas)
    ...
    for node_type, node_list in pairs(node_info) do
        insert(ret, format("# TYPE node_type gauge\n", node_type))
        for node, node_id in pairs(node_list) do
            insert(ret, format("node_%s{node=\"%s\"} %d\n", node_type, node,
                               node_id))
        end
    end
    while true do
        ...
        for name, content in pairs(data) do
            ...
            makecontent(ret, name, node, typestr, content)
            for _node, _data in pairs(datas) do
                local _content = _data[name]
                if _content then
                    _data[name] = nil
                    makecontent(ret, name, _node, typestr, _content)
                end
            end
        end
    end
