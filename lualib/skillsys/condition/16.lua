...
return function(bctx, self, tobj, ctx, parm, c_args)
    ...
    if stype == subtype_buffid then
        if get_buff_by_cfgid(tobj, id) then return true end
    elseif stype == subtype_buff_type then
        if get_buff_by_type(tobj, id) then return true end
    elseif stype == subtype_control then
        if check_control_buff(tobj, id) then return true end
    elseif stype == subtype_group then
        if check_exist_bygroup(tobj, id) then return true end
