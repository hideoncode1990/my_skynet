...
dict_open(lua_State *L) {
	...
	Table **ptr=lua_newuserdata(L,sizeof(*ptr));
	*ptr = dict;
	lua_createtable(L,0,1);
	lua_pushcfunction(L,dict_gc);
...
static Table * 
check_dict(lua_State *L,int idx){
	Table **ptr=lua_touserdata(L,idx);
	if(!ptr){
		luaL_error(L,"need Table but got a %s @ %d",luaL_typename(L,idx),idx);
	}
	return *ptr;
}
